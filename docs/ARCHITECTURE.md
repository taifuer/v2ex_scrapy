...
5. 通过 `content_families.json` 将版本词扩展到所属关键词组；主排名和板块仅展示父项，版本词继续保留搜索、独立趋势和代表帖子。
...
`content_detail_terms.txt` 独立维护可查看详情和搜索的稳定关键词。这些词仍需满足基础频次、作者数和节点数，但不会因此被强制放入各期 Top 排名。`content_synonyms.json` 只合并同一实体的大小写、空格和简称等写法；`content_families.json` 另行定义 GPT、AI Agent、Prompt 等展示关键词组，避免为了主榜去重而丢失 GPT-4、GPT-5 等版本变化信息。分词结果缓存在忽略的 `analysis/content_tokens.sqlite`；缓存按帖子 ID 和标题维护，词典、同义词或停用词变化会更新处理指纹，只重算新增、标题改变或配置失效的记录。`content_hotspot_audit.py` 会检查头部作者/节点集中度和示例标题，分词词表修改前应先审计语境，而不是只看词频。模型可辅助检查一份有频次、样例和集中度的有限候选报告，但候选必须回查标题并由人工决定，不能让模型直接修改词表或数据库。
