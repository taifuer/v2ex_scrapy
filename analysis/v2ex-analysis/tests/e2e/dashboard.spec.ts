...
test("restores and navigates the monthly data view", async ({ page }) => {
  ...
  await expect(monthlyView.locator(".monthly-data-grid .profile-rank-row")).toHaveCount(30)
  ...
  const topic = (await monthlyView.locator(".monthly-data-grid section").first().locator("button strong").first().textContent()) || ""
  await monthlyView.locator(".monthly-data-grid section").first().locator("button").first().click()
