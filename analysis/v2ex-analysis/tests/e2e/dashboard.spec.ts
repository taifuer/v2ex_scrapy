...
test("loads core views without runtime or layout errors", async ({ page }) => {
  ...
  const memberEvolutionWidth = await page.locator("#member-evolution").evaluate((chart) => ({
  ...
  expect(memberEvolutionWidth.canvas).toBe(memberEvolutionWidth.chart)
  await expect(page.getByLabel("成员排名指标").locator(".active")).toHaveText("发帖")
  await expect(page.locator("#member-evolution")).toHaveCSS("height", "412px")
  ...
  await expect(memberEvolution.locator(".ranked-column").first().locator(".ranked-item")).toHaveCount(10)
  await memberEvolution.getByLabel("成员排名指标").getByRole("button", { name: "评论", exact: true }).click()
  await expect(page.locator("#member-concentration canvas").first()).toBeVisible()
  await expect(page.getByLabel("头部成员范围").locator(".active")).toHaveText("Top 10")
  await page.getByLabel("头部成员范围").getByRole("button", { name: "Top 100", exact: true }).click()
  await expect(page.locator("#member-concentration")).toHaveAttribute("data-limit", "100")
  await expect.poll(() => new URL(page.url()).searchParams.get("memberShareTop")).toBe("100")
...
test("presents offline observations as a stateful HTML slide deck", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "下一页", exact: true }).click()
  await expect(page.locator(".deck-stage")).toHaveAttribute("data-slide", "overview")
...
test("restores a limited member profile from URL and browser history", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "参与方向变化", exact: true })).toBeVisible()
  await expect(page.locator("#member-direction canvas").first()).toBeVisible()
  await expect(page.getByLabel("成员参与行为").locator(".active")).toHaveText("发帖")
  await expect(page.getByLabel("成员参与维度").locator(".active")).toHaveText("话题")
  await page.getByLabel("成员参与行为").getByRole("button", { name: "评论", exact: true }).click()
  await page.getByLabel("成员参与维度").getByRole("button", { name: "标题关键词", exact: true }).click()
  await expect(page.locator("#member-direction canvas").first()).toBeVisible()
