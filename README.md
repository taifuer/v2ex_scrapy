...
V2EX 全站主题、评论和成员爬虫，附带按时间、话题、节点、成员和互动指标分析的 Vue 仪表盘。看板支持可分享 URL、月度与年度数据、事件注释、有限成员详情和离线社区观察。数据保存到根目录 `v2ex.sqlite`。
...
- 帖子：标签话题演变、话题详情、节点分布、节点详情、聚合话题和生命周期。节点详情按需加载全历史综合 Top 100 代表帖子并分页展示，话题、节点及有限成员之间支持内部下钻。
...
前端主包与 ECharts 图表运行时独立构建，互动、观察和节点详情也拆为独立视图代码块。月度和年度榜单仅加载当前所选月份或年份，评论活跃时段仅在数据概览中加载；话题趋势数据按年份读取，话题、节点和成员详情分别从 64 个哈希分片中按需加载自身趋势与代表内容，不依赖对应演变视图的全局 JSON。话题和成员演变使用固定宽度热力图与 ECharts 范围缩放，避免长时间区间生成超宽 Canvas。
