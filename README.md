...
V2EX 全站主题、评论和成员爬虫，附带按时间、话题、标题热词、节点、成员和互动指标分析的 Vue 仪表盘。看板支持可分享 URL、月度与年度数据、事件注释、有限成员详情和离线社区观察。数据保存到根目录 `v2ex.sqlite`。本项目为非官方社区数据项目。
...
- 帖子：话题演变、话题详情、节点分布、节点详情、标题内容热点、话题分类和生命周期。内容热点按标题中包含各热词的主题数展示每月或每年 Top 10/20/30；人工确认的领域词在任一完整月达到 8 个标题、5 位作者和 2 个节点，或任一年度达到 30 个标题、15 位作者和 2 个节点后，也可进入详情搜索，但不改变演变榜单。构建同时输出作者/节点集中度审计报告。生命周期使用统一 7 日窗口呈现参与用户数、每人评论数、楼主参与率和 `@` 提及率。节点详情按需加载全历史综合 Top 100 代表帖子并分页展示。
- 成员：成员增长、参与结构、逐期成员演变、累计贡献榜，以及有限成员的参与节点、发帖话题、标题内容、代表帖子和获感谢评论。
- 互动：点击、收藏、感谢、投票及标准化互动率。
- 观察：基于固定比较窗口生成话题结构、标题内容迁移、互动偏好与社区行为的离线解读，并保留证据链接和口径说明。
