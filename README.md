...
兼容入口：重建话题详情、分年度 Top 10 及分月 Top 3 代表帖子（均排除推广节点）：
...
前端主包与 ECharts 图表运行时独立构建，内容热点、互动、观察、节点详情、生命周期和概览趋势也拆为独立视图代码块。月度和年度榜单仅加载当前所选周期及必要的同比基线，评论活跃时段仅在数据概览中加载；话题与标题热点趋势按年份读取，热词、话题、节点和成员详情分别从 64 个哈希分片中按需加载，不依赖对应演变视图的全局 JSON。全局搜索的四类实体索引和近期候选仅在首次打开搜索时加载；空输入展示最近 12 个完整月份的 3 个热门话题和 3 个不重名内容热词。移动端默认收起全局筛选，每栏榜单先展示 10 项并可展开；热力图使用固定宽度和 ECharts 范围缩放，避免长时间区间生成超宽 Canvas。
