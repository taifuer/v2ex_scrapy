...
前端主包与 ECharts 图表运行时独立构建，互动、观察和节点详情也拆为独立视图代码块。月度和年度榜单仅加载当前所选月份或年份，评论活跃时段仅在数据概览中加载；话题趋势数据按年份加载当前筛选范围所需分片，话题和节点详情再按需读取对应哈希分片。话题和成员演变使用固定宽度热力图与 ECharts 范围缩放，避免长时间区间生成超宽 Canvas。
