...
V2EX 全站主题、评论和成员爬虫，附带按时间、话题、节点、成员和互动指标分析的 Vue 仪表盘。看板支持可分享 URL、月度与年度数据、事件注释、有限成员参与画像和离线社区观察。数据保存到根目录 `v2ex.sqlite`。
...
- 帖子：标签话题演变、独立话题详情、聚合话题、节点分布、生命周期和代表帖子。
...
前端主包与 ECharts 图表运行时独立构建。月度、年度、代表帖子和观察等无图表视图只加载轻量主包；话题趋势数据也按年份加载当前筛选范围所需分片。
