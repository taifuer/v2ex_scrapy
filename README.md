...
- 帖子：标签话题演变、包含代表帖子的独立话题详情、聚合话题、节点分布和生命周期。
...
前端主包与 ECharts 图表运行时独立构建。月度和年度榜单仅加载当前所选月份或年份，评论活跃时段仅在数据概览中加载，观察等无图表视图只加载轻量主包；话题趋势数据按年份加载当前筛选范围所需分片，话题详情再按需读取关联结构与代表帖子。话题和成员演变使用固定宽度热力图与 ECharts 范围缩放，避免长时间区间生成超宽 Canvas。
