...
V2EX 全站主题、评论和成员爬虫，附带按时间、标签话题、标题内容、节点、成员和互动指标分析的 Vue 仪表盘。看板支持可分享 URL、月度与年度数据、事件注释、有限成员详情和离线社区观察。数据保存到根目录 `v2ex.sqlite`。
...
仅更新标签详情分片中的代表帖子候选（每月全站 Top 30，并排除推广节点）：
...
- 帖子：标签话题演变、话题详情、节点分布、节点详情、标题内容热点、聚合话题和生命周期。内容热点按包含各词的主题标题数展示每月或每年 Top 10/20/30，详情提供趋势、关联标签、节点、活跃用户及每年 Top 10 代表帖子；节点详情按需加载全历史综合 Top 100 代表帖子并分页展示。
- 成员：成员增长、参与结构、逐期成员演变、累计贡献榜，以及有限成员的代表帖子和获感谢评论。
- 互动：点击、收藏、感谢、投票及标准化互动率。
- 观察：基于固定比较窗口生成内容结构、话题迁移、互动偏好与社区行为的离线解读，并保留证据链接和口径说明。

“月度”和“年度”位于概览的二级视图。月度支持环比与同比；当前年达到 2 个完整月份后，年度默认当前年并按相同月份范围同比，否则回退最近完整年。两类视图都展示热门话题、热门节点、活跃用户 Top 20，以及从对应周期全量数据中独立计算的四类帖子 Top 100 和感谢评论 Top 100。

前端主包与 ECharts 图表运行时独立构建，内容热点、互动、观察和节点详情也拆为独立视图代码块。月度和年度榜单仅加载当前所选月份或年份，评论活跃时段仅在数据概览中加载；标签与标题热点趋势按年份读取，词条、话题、节点和成员详情分别从 64 个哈希分片中按需加载，不依赖对应演变视图的全局 JSON。热力图使用固定宽度和 ECharts 范围缩放，避免长时间区间生成超宽 Canvas。
